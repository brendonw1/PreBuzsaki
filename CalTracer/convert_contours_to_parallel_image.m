...
inputcontours = handles.exp.contourLines;
currentim = handles.exp.tcImage.image;
...
hi = figure('visible','off');%make a stupid invisible figure so the closing
    %of the other fig will close it
waitfor(hi);%only progress once the invisible fig is closed
...
for icidx = 1:length(inputcontours);
    contourimage = imagefromcontours(inputcontours(icidx),size(handles.exp.tcImage.image));
...
% contoursimage = imagefromcontours(inputcontours,size(handles.exp.tcImage.image));
% registered = imtransform(contoursimage,mytform,...
%     'VData',[1 size(currentim,1)],'UData',[1 size(currentim,2)],...
%     'YData',[1 heightpi],'XData',[1 widthpi],...
%     'XYScale',[1 1]);
% % ,...
% %     'Size',[heightpi widthpi]);
% registeredcontours = findfrommatrix(registered, eps , 1 ,Inf);
% 
% masks = masksfromcontours(registeredcontours,[heightpi widthpi]);
% valsinside = zeros(1,length(masks));
% nummodevals = zeros(1,length(masks));
% for rcidx = 1:length(masks);
%     tempvar = registered(masks{rcidx});%get values inside
%     tempvar = tempvar(~(tempvar-round(tempvar)));%find just the integers inside
%     valsinside(rcidx) = mode(tempvar);%keep the mode of them
%     nummodevals(rcidx) = sum(tempvar==mode(tempvar));%as a tiebreaker, store the number of points with the mode value
% end
% 
% outputcontours = cell(1,length(inputcontours));
% for icidx = 1:length(inputcontours);
%     corresponding = find(valsinside == icidx);
%     if isempty(corresponding);
%         outputcontours{icidx} = Inf * ones(10,2);%make dummy contour to fill in
%            %NOTE these coordinates are not drawn when asked to be plotted
%            %as a line or a patch, but cause no error... apparently perfect
%            %for this
%     elseif length(corresponding)>1;%if more than one contour had icidx as it's mode
%         %break tie
%         [trash,idx] = max(nummodevals(corresponding));%find the one that had more of that mode value
%         corresponding = corresponding(idx);%tie broken, winner taken
%         outputcontours{icidx} = registeredcontours{corresponding};
%     elseif length(corresponding) == 1
%         outputcontours{icidx} = registeredcontours{corresponding};
%     end
% end

figure;imagesc(parallelim);colormap gray;plotfromcontours(outputcontours);
drawnow
