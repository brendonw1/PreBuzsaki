...
for idx=1:size(handles.exp.detections,2);
    match(idx)=strcmpi('ct_detectspikesintegrals',handles.exp.detections(idx).detectorName);
end
if sum(match)>0;
    match=find(match);
    ...
    %detection
    match=max(match);
    options.RawIntegralHardThreshHi.value = handles.exp.detections(match).params.RawIntegralHardThreshHi;
    options.RawIntegralTimesNoiseHi.value = handles.exp.detections(match).params.RawIntegralTimesNoiseHi;
    options.RawIntegralHardThreshLo.value = handles.exp.detections(match).params.RawIntegralHardThreshLo;
    options.RawIntegralTimesNoiseLo.value = handles.exp.detections(match).params.RawIntegralTimesNoiseLo;
    options.RiseIntegralHardThresh.value = handles.exp.detections(match).params.RiseIntegralHardThresh;
    ...
    options.RawIntegralTimesNoiseHi.value = min([100 handles.exp.fs*5]);%depends on frame rate ie number of samples
    options.RawIntegralHardThreshLo.value = .1;
    options.RawIntegralTimesNoiseLo.value = handles.exp.fs/2;%depends on frame rate ie number of samples
