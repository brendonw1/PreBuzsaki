...
%order = handles.exp.contourOrder(coidx).order;
...
handles.appData.partitions(pidx).displayedContours = displayed_contours;
...
if (do_signal_markers & didx > 0)
    ...
    for cid = displayed_contours
        ...
        cidx = find([handles.exp.contours.id] == cid);
        onsets = handles.exp.detections(didx).onsets(cidx);
        offsets = handles.exp.detections(didx).offsets(cidx);
        oncolor = [1 0 0];
        offcolor = [0 0 0];
        if ~isempty(onsets)
            onsets = onsets{1};
...
if (do_use_current_cell)
    active_cells = handles.appData.currentCellId;
else
...
if (~isempty(active_cells))
    for cid = active_cells
        ...
        cidx = find([handles.exp.contours.id] == cid);
        color = handles.exp.contourColors(index(cidx),:);
        height = find(cid == displayed_contours);
        if (~isempty(height))
            line(tracetime(1), ...
                height, ...
