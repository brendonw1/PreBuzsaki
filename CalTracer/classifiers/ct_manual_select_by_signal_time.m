...
if prod(size(handles.exp.detections))==1;%if no detections done yet
...
numFigClusters = handles.exp.partitions(pidx).numClusters;
partitionContours = [];
for cidx = 1:numFigClusters;
    partitionContours = [partitionContours handles.exp.partitions(pidx).clusters(cidx).contours];
end
...
for cid = 1:size(partitionContours,2);%for each cell
    ...
    if ~isempty(handles.exp.detections(didx).onsets{cidx})%if any signal detected in that trace
        temp = handles.exp.detections(didx).onsets{cidx}(1);%take the first onset to use
...
[x, y] = ginput;
sel_idxs = ceil(x * handles.exp.globals.fs);
sel_idxs = [0 ; sel_idxs];
