...
%    exp.preprocessOptions{1} = struct;
%    exp.preprocessOptions{2} = struct;
%    exp.preprocessOptions{3} = feval('ct_baseline_subtract_options');
     exp.preprocessOptions{1} = feval('ct_dfof_options');
     exp.preprocessOptions{2} = struct;
end

if (~isfield(exp, 'preprocessStrings'))
%    exp.preprocessStrings{1} = 'halo_subtract';
%    exp.preprocessStrings{2} = 'zscore' ;
%    exp.preprocessStrings{3} = 'baseline_subtract';
...
% Too much a pain in the ass. -DCS:2005/07/19
%if (isfield(exp, 'partitions') ...
%    & isfield(exp.partitions(1), 'numNeurons'))
%    for i = 1:length(p.partitions)
%	p(i) = newpartition;
%	p(i).id = 1;
%	p(i).title = '1';
%	p(i).clusters = exp.clusters;
%	for i = 1:length(p.clusters)
%	    p(i).clusters(i).numContours = p.clusters(i).numNeurons; 
%	end
%	rmfield(exp, 'clusters');
%	p.numClusters = exp.numClusters;
%	rmfield(exp, 'numClusters');
%	p.colors = exp.clusterColor;
%	rmfield(exp, 'clusterColor');
%	p.startIdxs = exp.cluster.startIdxs;
%	p.stopIdxs = exp.cluster.stopIdxs;
%	rmfield(exp, 'cluster');
%	exp.partitions = p;
%    end
%    appdata.currentPartitionIdx = 1;
%    appdata.numPartitions = 1;    
%end


% Attempt to be backward compatible here.
%if (~isfield(exp, 'partitions') & isfield(exp, 'clusters'))
%    p = newpartition;
%    p.id = 1;
%    p.title = '1';
%    p.clusters = exp.clusters;
%    for i = 1:length(p.clusters)
%       p.clusters(i).numContours = p.clusters(i).nNeurons; 
%    end
%    rmfield(exp, 'clusters');
%    p.numClusters = exp.numClusters;
%    rmfield(exp, 'numClusters');
