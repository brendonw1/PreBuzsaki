function active_cells_to_vnt(handles)
...
activecoords = handles.exp.centroids(actives);
targetsmatrix = reshape(cell2mat(activecoords),[2,length(actives)]);
defaultfiller = repmat(defaultfiller,[1, size(targetsmatrix,2)]);
targetsmatrix = [targetsmatrix;defaultfiller]; 

vntname = [handles.exp.fileName(1:end-4),'.vnt'];
