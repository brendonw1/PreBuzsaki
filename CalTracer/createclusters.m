...
all_contours = handles.exp.contours;
globals = handles.exp.globals;
...
% because all partitions pull data from handles.exp.contours, which
% changes with each preprocessing.  This means that each time we
% cluster we should reprocess the data.
handles.appData.isPreprocessedValid = 0;
if (~handles.appData.isPreprocessedValid)
    handles.exp.preprocessStrings = preprocess_strings;
    handles.exp.preprocessOptions = preprocess_options;
    [clean_contours, clean_halos] = preprocess(handles);
    % Since clean contours takes the raw data from handles.exp.contours
    ...
    if (handles.exp.haloMode)
        orig_clean_halos = clean_halos;
        clean_halos = clean_halos(clustered_contour_ids,:);
    end
    % The isPreprocessedValid should never be set!  Major mistake
    % to even think so!
    %handles.appData.isPreprocessedValid = 1;
end
...
% The 'p' for partition, which will be added to handles.exp at the end
% of the file.
newp = newpartition;
...
newp.clusterIdxsByContour = num2cell(NaN*ones(1,handles.exp.numContours));
for i = 1:newp.numClusters
    ...
    % array are indices into handles.exp.contours.  Put the contours
    ...
    order = handles.exp.contourOrder(coidx).order(contour_ids);
...
pidx = handles.exp.numPartitions + 1;
handles.exp.numPartitions = pidx;
newp.id = pidx;
...
handles.appData.partitions(pidx) = newpartition_appdata(newp.numClusters);
handles.exp.partitions(pidx) = newp;
