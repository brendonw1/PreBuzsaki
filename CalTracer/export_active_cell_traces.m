...
for pidx = 1:size(handles.exp.partitions,2)
    parnames{pidx} = handles.exp.partitions(pidx).title;
end
...
partition = handles.exp.partitions(parnum);

%% ask order
defaultOrder =  partition.contourOrderId;
orderlist = 1:size(handles.exp.contourOrder,2);
...
order = handles.exp.contourOrder(orderSelection).order;
...
actives = handles.appData.activeCells;

indices = handles.exp.contourOrder(orderSelection).index;
...
%% Export
moviename = handles.exp.fileName;
