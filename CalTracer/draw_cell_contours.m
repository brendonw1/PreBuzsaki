...
if (~do_save_handles)
    handles = delete_contour_handles(handles, 'ridx', ridx, 'midx', midx);
end
...
%     delete(face.handl{r}{midx});
    face.handl{r}{midx} = [];		% kill handles since redoing.
    if (do_special_color)
        cl = special_cl;
    else
        cl = handles.exp.regions.cl(r,:);
    end
...
numcells = size(handles.exp.regions.contours{handles.appData.currentRegionIdx}{handles.appData.currentMaskIdx},2);
set(handles.exp.tcImageTitle,'string',[handles.exp.fileName,'.  ',num2str(numcells),' cells.'])
%zoom on;
