...
do_use_raster = 1;
start_idxs = 1;
stop_idxs = length(handles.exp.contours(1).intensity);
if (isfield(options, 'doUseRaster') & ~options.doUseRaster.value)
	...
	% Add the halos because all the preprocessing routines are
        ...
        rastermap = [handles.exp.traces(clustered_contour_ids, :); ...
		     handles.exp.haloTraces(clustered_contour_ids,:)];
...
% set.
all_onsets = cell(1, handles.exp.numContours);
all_offsets = cell(1, handles.exp.numContours);
cidx = 1;
...
handles.exp.numDetections = handles.exp.numDetections + 1;
didx = handles.exp.numDetections;
handles.appData.currentDetectionIdx = didx;
handles.exp.detections(didx) = newdetection;
handles.exp.detections(didx).id = didx;
handles.exp.detections(didx).title =[signal_detector '_id' num2str(didx)];
handles.exp.detections(didx).detectorName = signal_detector_fun;
handles.exp.detections(didx).params = params;
handles.exp.detections(didx).onsets = all_onsets;
handles.exp.detections(didx).offsets = all_offsets;
