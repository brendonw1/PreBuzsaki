function [onsets, offsets, param] = ...
    epo_find_upstates(rastermap, handles, ridxs, clustered_contour_ids, options);


timesnoise = 2;%arbitrary number
minduration = .5;%minimum duration of an event, in seconds
minduration = minduration * handles.exp.fs;%now min duration in # of frames

%%detect overall noise level for the movie using the entire duration of all
%%traces
pidx = handles.appData.currentPartitionIdx;
wholetraces = handles.exp.partitions(pidx).cleanContourTraces;
