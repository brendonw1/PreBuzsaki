...
centroidA = handles.exp.contours(clean_contour_id).Centroid;
for i = 1:num_contours
    clean_traceB = clean_tracesB(i,:);
    centroidB = handles.exp.contours(contour_ids(i)).Centroid;    
    distances(i) = pdist([centroidA; centroidB], 'euclidean') * ...
        handles.exp.globals.mpp;
    [xcorrAB, lags] = xcorr(clean_traceA, clean_traceB);
    m = max(xcorrAB);
    m = m(1);
    maxlags(i) = lags(find(xcorrAB == m));
end
maxlags_secs = maxlags*handles.exp.globals.timeRes;
...
for i = 1:num_contours
    ...
    centroid_i = handles.exp.contours(all_contour_ids(i)).Centroid;
    for j = 1:num_contours
        clean_trace_j = all_clean_traces(j,:);
        centroid_j = handles.exp.contours(all_contour_ids(j)).Centroid;
...
max_lags_secs_mat = max_lag_mat * handles.exp.globals.timeRes;
