...
handles.appData.activeContourColor = [1 1 1];

% Rename some variables for ease.
% Current cell concept.
current_cell = handles.appData.currentCellId;
...
do_centroid_display = handles.appData.centroidDisplay.on;
...
% onsets = handles.exp.onsets;
% offsets = handles.exp.offsets;
...
all_contour_ids = [handles.exp.contours.id];
...
for cidx = 1:length(handles.appData.centroidDisplay.points)
    h1 = handles.appData.centroidDisplay.points(cidx);
    h2 = handles.appData.centroidDisplay.text(cidx);
...
handles.appData.centroidDisplay.points = [];
handles.appData.centroidDisplay.text = [];
...
if (~do_use_current_cell)    
    highlighted_cells = handles.appData.activeCells;
else
...
for nid = highlighted_cells
    ...
    set(cnt(nidx), ...
	'facecolor', handles.appData.activeContourColor);
	...
	color = handles.exp.contourColors(index(nidx),:);
        xc = handles.exp.centroids{nidx}(1);
        yc = handles.exp.centroids{nidx}(2);
	...
	if (do_centroid_display)
        ...
        handles.appData.centroidDisplay.points(end+1) = ...
            line(xc,yc,'Marker','x','color', color,'HitTest','off');
        handles.appData.centroidDisplay.text (end+1)= ...
            text(xc,yc,txt,...
...
if (do_id_display_all)
    for nidx = 1:handles.exp.numContours
	color = handles.exp.contourColors(index(nidx),:);
        xc = handles.exp.centroids{nidx}(1);
        yc = handles.exp.centroids{nidx}(2);
...
for i = 1:nclusters
    ...
    centroids_array = [handles.exp.contours(nids).Centroid];
...
if (do_contour_order_line & num_highlighted_cells > 1)
    ...
    for nidx = 1:num_highlighted_cells
        ...
        centroid = [handles.exp.contours(nid).Centroid];
        centroid = [handles.exp.contours(nid).Centroid];
