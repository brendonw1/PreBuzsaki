...
save('color_by_overlap_matrix','cellClusterAssignments');

cl=bw_colormap(max(length(chunknums)));%get a colorwheel for this number of cells (+1)
% cl(1,:)=[];%eliminate extra, so there is a ramping up of colors towards red (ie not circular)
% cl=fliplr(cl);%orient so red is most repeats
% cl = flipud(cl);
