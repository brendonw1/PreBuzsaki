function overlaps = InteractTsTrainMovieOverlap(moviecell,numreshuffs)
...
%                 perc = sum(sharedons)/minavail;
                ...
                if interact1+interact2 == 2 &&...
                        strcmp('ss',movinfo1.Protocol) && strcmp('ss',movinfo2.Protocol);
                ...
                elseif interact1+interact2 == 1 &&... 
                        (strcmp('s',movinfo1.Protocol) && strcmp('ss',movinfo2.Protocol) ||...
                        strcmp('ss',movinfo1.Protocol) && strcmp('s',movinfo2.Protocol))
                ...
                elseif interact1+interact2 == 0  &&...
                        strcmp('s',movinfo1.Protocol) && strcmp('s',movinfo2.Protocol);
...
figure('name','Overlaps of stimstim & stim vs stim & stim');
subplot(2,1,1);
...
hold on
bar(nnx,nny,'r')
title({'Red: Pairs of stim movies.  Blue: Stimstim - Stim pairs';...
    ['Stim-Stim: ',num2str(mean(overlaps.nonnonpercs)),'\pm',num2str(std(overlaps.nonnonpercs)),'.  Stimstim-Stim: ',num2str(mean(overlaps.intnonpercs)),'\pm',num2str(std(overlaps.intnonpercs)),'.'];...
    kstext})
xlabel('% overlap')
...
xlabel('Difference in % overlap: TvT-TvTT')
ylabel('Number of reshuffles')
title({'Histogram of differences between 10000 random resamples of TvT and TvTT datasets. Vs observed';...
...
title(['Stim-Stim.  ',num2str(nnpercentile),'th percentile.'])
...
xlabel('Percent overlap')
ylabel('Number of datasets')
title(['StimStim-Stim Overlap.  ',num2str(inpercentile),'th percentile.'])
...
xlabel('Percent overlap')
ylabel('Number of datasets')
title(['StimStim-StimStim Overlap.  ',num2str(iipercentile),'th percentile.'])
...
numslices = sidx;
title([num2str(numsignifdiffs),' out of ',num2str(length(spercentiles)),' slices showed significant difference in overlap between TvT and TvTT.'])
