function locksteps = InteractTSTrainMovieLockstep(moviecell,numreshuffs)
...
figure('name','locksteps of of stimstim & stim vs stim & stim');
subplot(2,1,1);
...
title({'Red: Pairs of stim movies.  Blue: Stimstim - Stim pairs';...
    ['Stim-Stim: ',num2str(mean(locksteps.nonnonpercs)),'\pm',num2str(std(locksteps.nonnonpercs)),'.  Stimstim-Stim: ',num2str(mean(locksteps.intnonpercs)),'\pm',num2str(std(locksteps.intnonpercs)),'.'];...
    kstext})
xlabel('% lockstep')
...
xlabel('Difference in % lockstep: TvT-TvTT')
...
title(['Stim-Stim lockstep.  ',num2str(nnpercentile),'th percentile.'])
...
xlabel('Percent lockstep')
ylabel('Number of datasets')
title(['StimStim-Stim lockstep.  ',num2str(inpercentile),'th percentile.'])
...
xlabel('Percent lockstep')
ylabel('Number of datasets')
title(['StimStim-StimStim lockstep.  ',num2str(iipercentile),'th percentile.'])
...
title([num2str(numsignifdiffs),' out of ',num2str(length(spercentiles)),' slices showed significant difference in lockstep between TvT and TvTT.'])
