function overlaps = InteractSpontMovieTotalNumbers(moviecell,numreshuffs)
    ...
    for midx1 = 1:nummovs
        eval(['movinfo1 = moviecell.',sname,'(midx1);'])
            if movinfo1.UpYN1 && movinfo2.UpYN2
                ...
                %gather number for "interact" and "non" cases
                %also for "spontstim" vs "spont" 
                %also for "ss" vs "s"
                % for each, after cycling through midx, see whether
                % distributions are sigificantly different means for each
                % slice
...
numslices = sidx;
title([num2str(numsignifdiffs),' out of ',num2str(numslices),' slices showed significant difference in overlap between SvS and SvSS.'])
