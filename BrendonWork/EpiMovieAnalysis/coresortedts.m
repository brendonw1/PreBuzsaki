function sorted=coresortedts(sorted,goodperslice,corethresh);
%this function not only adds core cell movies to the original sorted, but
%also adds movies of just tscore cells

% goodperslice=2;
% corethresh=1;%what proportion does a cell have to fire to be "core"
warning off MATLAB:conversionToLogical
warning off MATLAB:divideByZero
goodtstrain=zeros(size(sorted,2),1);
goodspont=zeros(size(sorted,2),1);
    ...
    if sum(goodtstrain(a,:),2)>=goodperslice;%if there is more than one movie with more than 10 cells coming on from slice "a"
        ...
        for b=1:size(tgoodthisslice,2)%for each good tstrain movie
            ...
            for c=1:size(sorted{a}.tstrain(tgoodthisslice(b)).ons,1);%for each frame
                t=zeros(1,size(sorted{a}.contours,2));
                t(find(corecells))=sorted{a}.tstrain(tgoodthisslice(b)).ons(c,(find(corecells)));
                ttcore(c,:)=t;
                t=zeros(1,size(sorted{a}.contours,2));
                t(find(othercells))=sorted{a}.tstrain(tgoodthisslice(b)).ons(c,(find(othercells)));
                ttother(c,:)=t;
            end
            sorted{a}.ttcore{b}=ttcore;
            sorted{a}.ttother{b}=ttother;
            sorted{a}.tcorecells{b}=find(corecells);
        end   
...
%         end
    ...
    if sum(goodspont(a,:),2)>=goodperslice;%if there is more than one movie with more than 10 cells coming on from slice "a"
        ...
        for b=1:size(sgoodthisslice,2)%for each good spont movie
            ...
            ssother=[];
            for c=1:size(sorted{a}.spont(sgoodthisslice(b)).ons,1);%for each frame
                s=zeros(1,size(sorted{a}.contours,2));
                s(find(corecells))=sorted{a}.spont(sgoodthisslice(b)).ons(c,(find(corecells)));
                sscore(c,:)=s;
                s=zeros(1,size(sorted{a}.contours,2));
                s(find(othercells))=sorted{a}.spont(sgoodthisslice(b)).ons(c,(find(othercells)));
                ssother(c,:)=s;
            end
            sorted{a}.sscore{b}=sscore;
            sorted{a}.ssother{b}=ssother;
            sorted{a}.scorecells{b}=find(corecells);
        end
...
%             end   
    ...
    if sum(goodspont(a,:),2)>=1 & sum(goodtstrain(a,:),2)>=1;
        tscorecells=((sumcollspont{a}.*sumcolltstrain{a})==1);
        nonzeros=zeros(1,size(sorted{a}.contours,2));
        nonzeros(find(sumcollspont{a}))=1;
        tsothercells=nonzeros.*~tscorecells;
        for b=1:size(sgoodthisslice,2)%for each good spont movie
            stscore=[];
            stsother=[];
            for c=1:size(sorted{a}.spont(sgoodthisslice(b)).ons,1);%for each frame
                s=zeros(1,size(sorted{a}.contours,2));
                s(find(tscorecells))=sorted{a}.spont(sgoodthisslice(b)).ons(c,(find(tscorecells)));
                stscore(c,:)=s;
                s=zeros(1,size(sorted{a}.contours,2));
                s(find(tsothercells))=sorted{a}.spont(sgoodthisslice(b)).ons(c,(find(tsothercells)));
                stsother(c,:)=s;
            end
            sorted{a}.stscore{b}=stscore;
            sorted{a}.stsother{b}=stsother;
            sorted{a}.tscorecells{b}=find(tscorecells);
        end
        for b=1:size(tgoodthisslice,2)%for each good tstrain movie
            ttscore=[];
            ttsother=[];
            for c=1:size(sorted{a}.tstrain(tgoodthisslice(b)).ons,1);%for each frame
                t=zeros(1,size(sorted{a}.contours,2));
                t(find(tscorecells))=sorted{a}.tstrain(tgoodthisslice(b)).ons(c,(find(tscorecells)));
                ttscore(c,:)=t;
                t=zeros(1,size(sorted{a}.contours,2));
                t(find(tsothercells))=sorted{a}.tstrain(tgoodthisslice(b)).ons(c,(find(tsothercells)));
                ttsother(c,:)=t;
            end
            sorted{a}.ttscore{b}=ttscore;
            sorted{a}.ttsother{b}=ttsother;
        end
    end
end