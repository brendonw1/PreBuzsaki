function basicmovieanalysis(sorted,goodperslice);
...
%             if sum(collwdtrain{c}(d,:),2)>=10;%if total number of cells on in this movie is greater than 5:
...
%     if sum(goodwdtrain(a,:),2)>=goodperslice;%if there is more than one movie with more than 10 cells coming on from slice "a"
...
[sliceoverlaps]=pairwiseoverlapsnaive(sorted,1);
ttmeanpairo=[];
ttsdpairo=[];
ssmeanpairo=[];
sssdpairo=[];
