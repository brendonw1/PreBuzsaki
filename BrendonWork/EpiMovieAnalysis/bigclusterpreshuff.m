function [tstraincluster, spontcluster, tstraincolumns, tstrainlayers, spontcolumns, spontlayers]=bigclusterpreshuff(sorted,shuffled);
%analyzes amount of clustering in collapsed movies, max frames of movies
%and frames before and after the max frame by calling on "evalclusteringpreshuff".
...
%                 goodtstrain(c,d)=1;%record that this movie was big enough
                [tstraincollcluster(end+1), tstraincollxcluster(end+1), tstraincollycluster(end+1), tstraincollpercentile(end+1)]=evalclusteringpreshuff(colltstrain{c}(d,:),sorted{c}.contours,shuffled{c}.tstrain(d).ons);
%                 maxframe=sum(sorted{c}.tstrain(d).ons,2);
%                 [garbage,maxframe]=max(maxframe);
%                 if sum(sorted{c}.tstrain(d).ons(maxframe,:))>5;
%                     [tstrainmaxcluster(end+1), tstrainmaxxcluster(end+1), tstrainmaxycluster(end+1), tstrainmaxpercentile(end+1)]=evalclusteringpreshuff(sorted{c}.tstrain(d).ons(maxframe,:),sorted{c}.contours);
%                 end
%                 if maxframe+1<=size(sorted{c}.tstrain(d).ons,1) & sum(sorted{c}.tstrain(d).ons(maxframe+1,:))>=5;
%                     [tstrainmaxp1cluster(end+1), tstrainmaxp1xcluster(end+1), tstrainmaxp1ycluster(end+1), tstrainmaxp1percentile(end+1)]=evalclusteringpreshuff(sorted{c}.tstrain(d).ons(maxframe+1,:),sorted{c}.contours);
...
%                     [tstrainmaxp2cluster(end+1), tstrainmaxp2xcluster(end+1), tstrainmaxp2ycluster(end+1), tstrainmaxp2percentile(end+1)]=evalclusteringpreshuff(sorted{c}.tstrain(d).ons(maxframe+2,:),sorted{c}.contours);
...
%                     [tstrainmaxm1cluster(end+1), tstrainmaxm1xcluster(end+1), tstrainmaxm1ycluster(end+1), tstrainmaxm1percentile(end+1)]=evalclusteringpreshuff(sorted{c}.tstrain(d).ons(maxframe-1,:),sorted{c}.contours);
...
%                     [tstrainmaxm2cluster(end+1), tstrainmaxm2xcluster(end+1), tstrainmaxm2ycluster(end+1), tstrainmaxm2percentile(end+1)]=evalclusteringpreshuff(sorted{c}.tstrain(d).ons(maxframe-2,:),sorted{c}.contours);
...
%                 goodspont(c,d)=1;%record that this movie was big enough
                [spontcollcluster(end+1), spontcollxcluster(end+1), spontcollycluster(end+1), spontcollpercentile(end+1)]=evalclusteringpreshuff(collspont{c}(d,:),sorted{c}.contours,shuffled{c}.spont(d).ons);
%                 maxframe=sum(sorted{c}.spont(d).ons,2);
%                 [garbage,maxframe]=max(maxframe);
%                 if sum(sorted{c}.spont(d).ons(maxframe,:))>5;
%                     [spontmaxcluster(end+1), spontmaxxcluster(end+1), spontmaxycluster(end+1), spontmaxpercentile(end+1)]=evalclusteringpreshuff(sorted{c}.spont(d).ons(maxframe,:),sorted{c}.contours);
%                 end
%                 if maxframe+1<=size(sorted{c}.spont(d).ons,1) & sum(sorted{c}.spont(d).ons(maxframe+1,:))>=5;
%                     [spontmaxp1cluster(end+1), spontmaxp1xcluster(end+1), spontmaxp1ycluster(end+1), spontmaxp1percentile(end+1)]=evalclusteringpreshuff(sorted{c}.spont(d).ons(maxframe+1,:),sorted{c}.contours);
...
%                     [spontmaxp2cluster(end+1), spontmaxp2xcluster(end+1), spontmaxp2ycluster(end+1), spontmaxp2percentile(end+1)]=evalclusteringpreshuff(sorted{c}.spont(d).ons(maxframe+2,:),sorted{c}.contours);
...
%                     [spontmaxm1cluster(end+1), spontmaxm1xcluster(end+1), spontmaxm1ycluster(end+1), spontmaxm1percentile(end+1)]=evalclusteringpreshuff(sorted{c}.spont(d).ons(maxframe-1,:),sorted{c}.contours);
...
%                     [spontmaxm2cluster(end+1), spontmaxm2xcluster(end+1), spontmaxm2ycluster(end+1), spontmaxm2percentile(end+1)]=evalclusteringpreshuff(sorted{c}.spont(d).ons(maxframe-2,:),sorted{c}.contours);
