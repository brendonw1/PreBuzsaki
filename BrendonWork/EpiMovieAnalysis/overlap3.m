function [scores,sact,tcores,tact]=overlap3(sorted);
...
for a=1:size(sorted,2);%for each slice
    if sum(goodtstrain(a,:),2)>=goodperslice;%if there is more than one movie with more than 10 cells coming on from slice "a"
        ...
        tt(a).whichcore=find(sumcolltstrain{a}>=(1));%find cells on 2/3 or more (less than 100%)
    ...
    if sum(goodspont(a,:),2)>=goodperslice;%if there is more than one movie with more than 10 cells coming on from slice "a"
        ...
        ss(a).whichcore=find(sumcollspont{a}>=(1));%find cells on 2/3 or more (less than 100%)  
...
for a=1:size(sorted,2);
    scores=scores+length(ss(a).whichcore);
    sact=sact+length(ss(a).whichactive);
    tcores=tcores+length(tt(a).whichcore);
    tact=tact+length(tt(a).whichactive);
end    
% 
