function [realt,reals,realts,averages]=comborepeats3(sorted,goodperslice);
...
%                     compare=ones(size(sorted{a}.tstrain(p(u,1)).ons));%establish a matrix that will be continuously compared
                    for v=1:size(p,2);%for every index in that comparison, we'll do serial comparisons of movies
                        ...
                        availons(end+1)=sum(logical(sum(sorted{a}.tstrain(movienumber).ons)));    
                    end
...
%             end
    ...
    if n>=goodperslice;%if there is more than one movie with more than cellspermovie cells coming on from slice "a"
         g=find(goods(a,:));
         mo=min([n 5]);%go up to number of good spont movies, or 5, whichever is less.
         for order=1:mo;%for every good movie
            ind=nchoosek(1:n,order);
            average=[];
            p=ind;
...
%                     compare=ones(size(sorted{a}.spont(p(u,1)).ons));%establish a matrix that will be continuously compared
                    for v=1:size(p,2);%for every index in that comparison, we'll do serial logical multiplications of collapsed movies
                        movienumber=g(p(u,v));%take the number of the movie specified by finding which good movie it represents.                        
                        if v==1;
                        ...
                        availons(end+1)=sum(logical(sum(sorted{a}.spont(movienumber).ons)));
                    end
                    numer=sum(logical(sum(compare)));%how many cells overlapped between these
                    denom=min(availons);%find the max number of possible overlaps, based on finding the movie with the fewest cells on
                    average(end+1)=numer/denom;
...
%                         comparet=ones(size(sorted{a}.tstrain(pt(u,1)).ons));%establish a matrix that will be continuously compared
                        for v=1:size(pt,2);%for every index in that comparison, we'll do serial logical multiplications of collapsed movies
                            ...
                            availonst(end+1)=sum(logical(sum(sorted{a}.tstrain(movienumber).ons)));
                            ps=whichspont;
...
%                                 ps=perms(whichspont(xx,:));
                                for x=1:size(ps,1);
                                    ...
                                    for y=1:size(ps,2);%for every index of that combo
                                        ...
                                        availonss(end+1)=sum(logical(sum(sorted{a}.spont(movienumber).ons)));
                                    end
