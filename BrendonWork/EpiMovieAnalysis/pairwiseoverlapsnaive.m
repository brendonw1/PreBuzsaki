function [sliceoverlaps]=pairwiseoverlapsnaive(sorted,goodperslice);
...
%             if (sum(collg{c}(d,:),2)<10) & (sum(collg{c}(d,:),2)>0);%if total number of cells on in this movie is greater than 10:
...
%             if sum(collw{c}(d,:),2)>=10;%if total number of cells on in this movie is greater than 5:
...
for a=1:size(sorted,2);%for each slice
    ...
    sliceoverlaps(a).tw=[];
    sliceoverlaps(a).sw=[];
    if sum(goodt(a,:),2)>=goodperslice;%if there is more than one movie with more than 10 cells coming on from slice "a"
        g=find(goodt(a,:));
    ...
    if sum(goods(a,:),2)>=goodperslice;%if there is more than one movie with more than 10 cells coming on from slice "a"
        g=find(goods(a,:));
        for b=1:length(g);%for each good movie 
            for c=b+1:length(g);%for each not yet compared movie
    ...
    if sum(goodw(a,:),2)>=goodperslice;%if there is more than one movie with more than 10 cells coming on from slice "a"
        g=find(goodw(a,:));
        for b=1:length(g);%for each good movie 
            for c=b+1:length(g);%for each not yet compared movie
    ...
    if sum(goods(a,:),2)>=goodperslice & sum(goodt(a,:),2)>=goodperslice;%if both spont and stim in this slice have at least one good movie
        gs=find(goods(a,:));
        gt=find(goodt(a,:));
        for b=1:length(gt);
            for c=1:length(gs);
    ...
    if sum(goodt(a,:),2)>=goodperslice & sum(goodw(a,:),2)>=goodperslice;%if both spont and stim in this slice have at least one good movie
        gt=find(goodt(a,:));
        gw=find(goodw(a,:));
        for b=1:length(gt);
            for c=1:length(gw);
    ...
    if sum(goods(a,:),2)>=goodperslice & sum(goodw(a,:),2)>=goodperslice;%if both spont and stim in this slice have at least one good movie
        gs=find(goods(a,:));
        gw=find(goodw(a,:));
        for b=1:length(gs);
            for c=1:length(gw);
