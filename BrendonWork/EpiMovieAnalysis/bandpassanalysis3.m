function ons=bandpassanalysis3(pixels,contours,lengths);
...
for a=1:size(df,3);%for each frame in the series
    ...
    df3{a}=frame;
%     df10{a}=BWbpimage2(frame,3,10*meandiam);%filter image according to the size of the average contour in that image
    df2{a}=BWbpimage2(frame,small,big);%filter image according to the size of the average contour in that image
end
...
%         movie10=[];
        ...
        for c=1:dflengths(b);%for each frame in that movie
            frame2=df2{beginmovies(b)+c-1};%store the filtered frame of interest
            temp=template;%take template of non-cell pixels for manipulations in next few lines
            temp(:,colselim{beginmovies(b)+c-1})=[];%eliminate parts of template that are not relevant for this frame (according to which lines were eliminated)
            temp(rowselim{beginmovies(b)+c-1},:)=[];%same but other dimension
            frame2=frame2(logical(temp))';%vector of pixel values from non-cell areas
            ?template2=find(temp);%make a vector of non-cell pixels for this particular frame keep template 2
%             frame2=frame2(template2)';%vector of pixel values from non-cell areas
            ?framenoise2(c)=std(frame2);%find the noise of the non-cell pixels of that frame
            movie2=cat(2,movie2,frame2);%creating a vector of pixel values across the whole movi
...
%             movie10=cat(2,movie10,frame10);%creating a vector of pixel values across the whole movie 
            frame3=df3{beginmovies(b)+c-1};%ALL "3" things calculated here refer to original df frames (minus edges) instead of filtered images
            frame3=frame3(logical(temp))';%linear version of pixel values for this frame
...
%             fc=cat(2,fc2,fc10);
            if ~isempty(fc2);%if something detected
