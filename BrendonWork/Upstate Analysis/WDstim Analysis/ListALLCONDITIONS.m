function results = ListALLCONDITIONS(matnotes,stimnum,varargin);
...
results.wdtraincellnumbers = [];
results.wdtraincellnames = {};
results.wdtraincelldescrip = {};
results.burstbcellnumbers = [];
results.burstbcellnames = {};
results.burstbcelldescrip = {};
results.anyburstcellnumbers = [];
results.anyburstcellnames = {};
results.anyburstcelldescrip = {};
results.spontcellnumbers = [];
results.spontcellnames = {};
results.spontcelldescrip = {};
results.tstraincellnumbers = [];
results.tstraincellnames = {};
results.tstraincelldescrip = {};

results.diwdtraincellnumbers = [];
results.diwdtraincellnames = {};
results.diwdtraincelldescrip = {};
results.diburstbcellnumbers = [];
results.diburstbcellnames = {};
results.diburstbcelldescrip = {};
results.dianyburstcellnumbers = [];
results.dianyburstcellnames = {};
results.dianyburstcelldescrip = {};
results.dispontcellnumbers = [];
results.dispontcellnames = {};
results.dispontcelldescrip = {};
results.ditstraincellnumbers = [];
results.ditstraincellnames = {};
results.ditstraincelldescrip = {};

results.nondiwdtraincellnumbers = [];
results.nondiwdtraincellnames = {};
results.nondiwdtraincelldescrip = {};
results.nondiburstbcellnumbers = [];
results.nondiburstbcellnames = {};
results.nondiburstbcelldescrip = {};
results.nondianyburstcellnumbers = [];
results.nondianyburstcellnames = {};
results.nondianyburstcelldescrip = {};
results.nondispontcellnumbers = [];
results.nondispontcellnames = {};
results.nondispontcelldescrip = {};
...
%% To count for any interaction WDspontstim or BurstBurst
%                 if strcmp(matnotes(sidx).trial(tidx).stim,'wdtrain')%if the trial is 'wdtrain'
                    in6 = matnotes(sidx).trial(tidx).ephys.in6;
                    if isfield(matnotes(sidx).trial(tidx).ephys,'cell');
                        interactiontype = matnotes(sidx).trial(tidx).ephys.cell(cell).interactiontype;
...
%% to record data for each cell
        ...
        if thiscellhaswdtrain | thiscellhasburstb | thiscellhasanyburst|  thiscellhasspont | thiscellhaststrain;
            cellrecorded = 0;
            ...
            if thiscellhaswdtrain;
                results.wdtraincellnumbers(end+1,:) = [sidx cell];
                results.wdtraincellnames{end+1,1} = [sln,' ',cfn];
                results.wdtraincelldescrip{end+1,1} = [sln,' ',cfn,' ',num2str(celldi),' ',ov,' ',sp,' ',mo];
                if celldi
                    results.diwdtraincellnumbers(end+1,:) = [sidx cell];
                    results.diwdtraincellnames{end+1,1} = [sln,' ',cfn];
                    results.diwdtraincelldescrip{end+1,1} = [sln,' ',cfn,' ',num2str(celldi),' ',ov,' ',sp,' ',mo];
                else
                    results.nondiwdtraincellnumbers(end+1,:) = [sidx cell];
                    results.nondiwdtraincellnames{end+1,1} = [sln,' ',cfn];
                    results.nondiwdtraincelldescrip{end+1,1} = [sln,' ',cfn,' ',num2str(celldi),' ',ov,' ',sp,' ',mo];
                end
            end
            ...
            if thiscellhasanyburst;
                results.anyburstcellnumbers(end+1,:) = [sidx cell];
                results.anyburstcellnames{end+1,1} = [sln,' ',cfn];
                results.anyburstcelldescrip{end+1,1} = [sln,' ',cfn,' ',num2str(celldi),' ',ov,' ',sp,' ',mo];
                if celldi
                    results.dianyburstcellnumbers(end+1,:) = [sidx cell];
                    results.dianyburstcellnames{end+1,1} = [sln,' ',cfn];
                    results.dianyburstcelldescrip{end+1,1} = [sln,' ',cfn,' ',num2str(celldi),' ',ov,' ',sp,' ',mo];
                else
                    results.nondianyburstcellnumbers(end+1,:) = [sidx cell];
                    results.nondianyburstcellnames{end+1,1} = [sln,' ',cfn];
                    results.nondianyburstcelldescrip{end+1,1} = [sln,' ',cfn,' ',num2str(celldi),' ',ov,' ',sp,' ',mo];
                end
            end
