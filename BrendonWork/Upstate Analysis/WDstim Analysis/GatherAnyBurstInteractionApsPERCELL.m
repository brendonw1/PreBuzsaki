function results = GatherAnyBurstInteractionApsPERCELL(matnotes,stimnum,varargin);
...
poststimbin = (-beforetime/binwidth)+1;
prestimmean = mean(ycellrates(:,1:poststimbin-1),2);
ycr21 = [prestimmean,ycellrates(:,poststimbin)];
...
%% figure of reshuffles
% reshuffle which cells are in the population to look at the expectable
% variation for both higher and lower values
resamprates = resampledataset(cellrates(:,poststimbin),10000);%resample values of each cell from poststim bin
resampratemeans = mean(resamprates,1);
figure;
hist(resampratemeans,100);
s = sort(resampratemeans);
lowsignif = s(round(.05*length(s)));
highsignif = s(round(.95*length(s)));
hold on
plot([lowsignif lowsignif],[0 300],'color','g')
plot([highsignif highsignif],[0 300],'color','g')
plot(mean(prestimmean),0,'*','color','red')
title({[num2str(length(s)),' resamples of cells contributing to bin 1 in PSTH.  ',num2str(binwidth/10),'ms bins.'];...
    ['Mean=',num2str(mean(s)),' SD=',num2str(std(s)),'.  Range = ',num2str(min(s)),' to ',num2str(max(s))...
    '.  5th%ile=',num2str(lowsignif),' 95th%ile=',num2str(highsignif)];...
    ['Prestim bins mean=',num2str(mean(prestimmean))]})
