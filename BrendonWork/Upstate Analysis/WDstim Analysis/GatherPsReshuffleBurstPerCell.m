function results = GatherPsReshuffleBurstPerCell(matnotes,stimnum,binwidths,numreshuffs,varargin);
...
title({['Mean P value across First bins for each cell.  ',num2str(size(firstps,2)),' cells.'];...
    ['Bins = ',num2str(binwidths),'ms']});
figure;
hist(mean(results.maxps),100)
title({['Mean P value across Max bins for each cell.  ',num2str(size(firstps,2)),' cells.'];...
    ['Bins = ',num2str(binwidths),'ms']});
figure;
hist(max(results.firstps),100)
title({['Max P value across First bins for each cell.  ',num2str(size(firstps,2)),' cells.'];...
    ['Bins = ',num2str(binwidths),'ms']});
figure;
hist(max(results.maxps),100)
title({['Max P value across Max bins for each cell.  ',num2str(size(firstps,2)),' cells.'];...
    ['Bins = ',num2str(binwidths),'ms']});