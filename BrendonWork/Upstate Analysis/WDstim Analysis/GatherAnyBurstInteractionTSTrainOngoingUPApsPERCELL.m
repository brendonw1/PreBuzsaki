function results = GatherAnyBurstInteractionTSTrainOngoingUPApsPERCELL(matnotes,stimnum,binwidth,beforetime,aftertime,varargin);
...
%% Evaluate ongoing spiking in any UP state for same cell
            sallbutaps = 0;
            scellaps = [];
            scellevents = 0;
            scelluptimetotal = 0;
            ...
            sallcellapsonce = [];
            for tidx = 1:size(matnotes(sidx).trial,2)%go through each trial
                if strcmp(matnotes(sidx).trial(tidx).stim,'spont') | strcmp(matnotes(sidx).trial(tidx).stim,'tstrain')%if the trial is 'spont' or 'tstrain'
%                 if strcmp(matnotes(sidx).trial(tidx).stim,'spont')%if the trial is 'spont' or 'tstrain'
                    if isfield(matnotes(sidx).trial(tidx).ephys,'cell');
...
%% store num spikes and rate for each cell that had all 3
            if ycellevents & scellevents & tcellevents
                ...
                ascellupratebytotal(end+1) = scellupratebytotal(end);
                ascelluprateconserv(end+1) = scelluprateconserv(end);
...
sponts = ascellupratebytotal';
tstrains = atcellrates(:,poststimbin);
summedts = sponts+tstrains;
interacts = aycellrates(:,poststimbin);
% errorbargraph([mean(sponts) mean(tstrains) mean(summedts) mean(interacts)],[sem(sponts) sem(tstrains) sem(summedts) sem(interacts)],.6);
% if NO2
%     title ({['Spike Rates in ',num2str(binwidth/10),'ms post stim (Error bars = SEM over cells) NO 2 CELLS'];'Ongoing, Stim, Sum Ongoing+Stim, Interaction'})
% else
%     title ({['Spike Rates in ',num2str(binwidth/10),'ms post stim (Error bars = SEM over cells)'];'Ongoing, Stim, Sum Ongoing+Stim, Interaction'})
% end
% 
...
% xlabel('Summed Ongoing+Stim Rate')
% ylim([0 overallmax])
% xlim([0 overallmax])
% if NO2
%     title({['Expected vs Observed spiking of Ongoing+Stim.  ',num2str(binwidth/10),'ms Bins.  NO 2 CELLS'];'Red Line: Slope = 1'})
% else
%     title({['Expected vs Observed spiking of Ongoing+Stim.  ',num2str(binwidth/10),'ms Bins.'];'Red Line: Slope = 1'})
% end
% 
% 
tstrainbins = mean(tcellrates,1);
% spontbins = mean(scellrates,1);
spontbins = mean(scellupratebytotal)*ones(size(tstrainbins));
...
barerrors(1) = sem(scellrates(:,poststimbin));
barerrors(2) = sem(tcellrates(:,poststimbin));
barerrors(3) = 0;
barerrors(4) = sem(ycellrates(:,poststimbin));
% 
% % fig 3 (bar graphs of population expected vs observed.  spont = spont after pseudostim)
% % figure;
% % bar([.5 1.5 2.5 3.5],bardisplay,.6)
% errorbargraph(bardisplay,barerrors)
% if NO2
%     title({['Spike Rates in ',num2str(binwidth/10),'ms bins after stim.  +/- SEM.  NO 2 CELLS'];...
%         '1)=Ongoing Mean (T or S)  2)=DOWN Stim  3)=1+2  4)=Observed interaction'})
% else
%     title({['Spike Rates in ',num2str(binwidth/10),'ms bins after stim.  +/- SEM. '];...
%         '1)=Ongoing Mean (T or S)  2)=DOWN Stim  3)=1+2  4)=Observed interaction'})
% end
% 
...
title(['Ongoing Spont or TSTrain.  ',num2str(binwidth/10),'ms bins'])
subplot(2,2,2);%tstrain
bar(xs,tstrainbins,1);
line([-beforetime -beforetime],[0 100],'color','r');ylim([0 6])
title(['DOWN State Stim.  ',num2str(binwidth/10),'ms bins'])
subplot(2,2,3);%summed
bar(xs,summedbins,1);
line([-beforetime -beforetime],[0 100],'color','r');ylim([0 6])
title(['Ongoing+Stim. First bin:',num2str(summedbins(poststimbin)),'.']);
