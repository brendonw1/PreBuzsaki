function results = ReshuffleGatherAnyInteractionApsPERCELL(matnotes,stimnum,binwidth,numreshuffs,varargin);
...
%                 latmean = mean(ycelllatencies{end})/10;
%                 latrange = [min(ycelllatencies{end})/10 max(ycelllatencies{end})/10];
%                 latencystr = ['Mean Latency=',num2str(latmean),' RANGE: ',num2str(latrange(1)),'-',num2str(latrange(2)),'ms.'];
                latencystr = [];
                titlestr = {teststr;cellinfostr;numbersstr;[latencystr,'   ',signifstr]};
                title(titlestr);
            ...
            %% for plotting maxbin comparison data for this cell
                ...
                cellinfostr = [ycellnames{end},'.  ',...
                    'Slice ',num2str(sidx),' Cell ',num2str(cell),'.  ',num2str(size(upslist,1)),' trials.'];
                numbersstr = ['Cell: ',num2str(nummaxbinaps(end)),' spikes in max bin.  ',...
                    'Reshuffled distribution: mean = ',num2str(mean(thesereshuffs)),', sd = ',num2str(std(thesereshuffs)),'.'];
                signifstr = ['p = ',num2str(p),'.  ',num2str(numreshuffs),' reshuffles.'];
                ...
                %% for plotting pre-(neg)-bin comparison data for this cell
                thesereshuffs = znumnegbinaps(end,:);
                f = figure;
                hist(thesereshuffs);
                hold on
                plot(numnegbinaps(end),0,'*','color','r');
                p = sum(numnegbinaps(end)<=thesereshuffs)/numreshuffs;
                negps(end+1) = p;

                teststr = ['#APs in Preceding bin for cell (red *) vs Reshuffles (bars).  ',num2str(binwidth/10),'ms bins.'];
                cellinfostr = [ycellnames{end},'.  ',...
                    'Slice ',num2str(sidx),' Cell ',num2str(cell),'.  ',num2str(size(upslist,1)),' trials.'];
                numbersstr = ['Cell: ',num2str(numnegbinaps(end)),' spikes in preceding bin.  ',...
                    'Reshuffled distribution: mean = ',num2str(mean(thesereshuffs)),', sd = ',num2str(std(thesereshuffs)),'.'];
                signifstr = ['p = ',num2str(p),'.  ',num2str(numreshuffs),' reshuffles.'];
                titlestr = {teststr;cellinfostr;numbersstr;signifstr};
                title(titlestr);
                ...
                thisstr(strfind(thisstr,' '))=[];
                thisstr(end-3:end)=[];
                hgsave(f,[directory,'\',thisstr,'NegBin']);
            end
        end
    end
end
...
numbersstr = ['Pop: ',num2str(sum(nummaxbinaps)),' spikes in max bin.  ',...
    'Reshuffled distribution: mean = ',num2str(mean(thesereshuffs)),', sd = ',num2str(std(thesereshuffs)),'.'];
signifstr = ['p = ',num2str(p),'.  ',num2str(numreshuffs),' reshuffles.'];
titlestr = {teststr;cellinfostr;numbersstr;signifstr};
title(titlestr);
...
%% for plotting maxbin comparison data for whole population
thesereshuffs = sum(znumnegbinaps,1);
f = figure;
hist(thesereshuffs);
hold on
plot(sum(numnegbinaps),0,'*','color','r');
p = sum(sum(numnegbinaps)<=thesereshuffs)/numreshuffs;

teststr = ['#APs in NEG bin for Population (red *) vs Reshuffles (bars).  ',num2str(binwidth/10),'ms bins.'];
cellinfostr = [num2str(size(ycellnames,2)),' total cells.  ', num2str(size(numnegbinaps,2)),' cells spiking in these trials'];
numbersstr = ['Pop: ',num2str(sum(numnegbinaps)),' spikes in preceding bin.  ',...
    'Reshuffled distribution: mean = ',num2str(mean(thesereshuffs)),', sd = ',num2str(std(thesereshuffs)),'.'];
signifstr = ['p = ',num2str(p),'.  ',num2str(numreshuffs),' reshuffles.'];
titlestr = {teststr;cellinfostr;numbersstr;signifstr};
title(titlestr);
