function spikesreshuffuptraces=makespikesreshuffuptraces2(uptraces,numreshuff);
%THIS DIFFERS FROM MAKESPIKESRESHUFFUPTRACES IN THAT IT USES RESHUFFLESPIKEISIS2, NOT ONE.
%THIS SECOND VERSION OF THIS FUNCTION DOES NOT RANDOMLY PICK A
			...
			for d=1:size(uptraces,4);
				if uptraces(1).guide(a,b,c,d);
					...
					if ~isempty(aps)
						...
						for z=1:numreshuff;
							spikesreshuffuptraces(a,b,c,d).spikes(z,:)=reshufflespikeisis2(aps,start,stop);
						end
					end
				end
			end
